...
mejs.HtmlMediaElementShim = {
  ...
  determinePlayback: function (a, b, c, d, e) {
      ...
      for (g = 0; g < a.childNodes.length; g++) {
        ...
        if (h.nodeType == 1 && h.tagName.toLowerCase() == 'source') {
          ...
          if (
            !h ||
            !window.matchMedia ||
            (window.matchMedia && window.matchMedia(h).matches)
          )
            f.push({ type: l, url: e });
        }
      }
    if (
      ...
      f.length > 0 &&
      f[0].url !== null &&
      this.getTypeFromFile(f[0].url).indexOf('audio') > -1
    )
      j.isVideo = false;
    if (mejs.MediaFeatures.isBustedAndroid)
    ...
    if (
      ...
      for (g = 0; g < f.length; g++)
        if (
          a.canPlayType(f[g].type).replace(/no/, '') !== '' ||
          a.canPlayType(f[g].type.replace(/mp3/, 'mpeg')).replace(/no/, '') !==
            ''
        ) {
          j.method = 'native';
          j.url = f[g].url;
      ...
      for (g = 0; g < f.length; g++) {
        ...
        for (a = 0; a < b.plugins.length; a++) {
          ...
          for (c = 0; c < h.length; c++) {
            ...
            if (
              k.version == null ||
              mejs.PluginDetector.hasPluginVersion(e, k.version)
            )
...
(function (a, b) {
  ...
  c.methods.t = function (d, e, f) {
    if (
      c.locale.strings &&
      c.locale.strings[f.context] &&
      c.locale.strings[f.context][d]
    )
...
(function (f) {
  ...
  f.extend(MediaElementPlayer.prototype, {
    buildvolume: function (a, b, c, e) {
      if (
        !(mejs.MediaFeatures.hasTouch && this.options.hideVolumeOnTouchDevices)
      ) {
        var d = this,
